...
ALTER TABLE `machine_barcode`
  ...
  ADD COLUMN IF NOT EXISTS `product_trumpet` varchar(64) DEFAULT NULL COMMENT '机器小号' AFTER `product_name`,
  ADD COLUMN IF NOT EXISTS `machine_no` varchar(64) DEFAULT NULL COMMENT '机器小号' AFTER `product_model`,
  ADD COLUMN IF NOT EXISTS `scan_date` datetime DEFAULT NULL COMMENT '厂家最后出库日期' AFTER `brand_code`,
