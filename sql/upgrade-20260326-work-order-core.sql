...
CREATE TABLE IF NOT EXISTS `work_order` (
  ...
  `order_no`                    varchar(32)      NOT NULL                COMMENT '工单�?,
  `customer_id`                 bigint unsigned  DEFAULT NULL            COMMENT '客户ID',
  `customer_name`               varchar(64)      NOT NULL                COMMENT '客户姓名',
  `customer_mobile`             varchar(20)      NOT NULL                COMMENT '客户手机�?,
  ...
  `warranty_status`             varchar(16)      DEFAULT NULL            COMMENT '质保状�?,
  `fault_desc`                  text             DEFAULT NULL            COMMENT '客户报修描述',
  `sender_name`                 varchar(64)      DEFAULT NULL            COMMENT '寄件人姓�?,
  `sender_mobile`               varchar(20)      DEFAULT NULL            COMMENT '寄件人手机号',
  `sender_address`              varchar(255)     DEFAULT NULL            COMMENT '寄件地址',
  `send_express_no`             varchar(64)      DEFAULT NULL            COMMENT '寄件快递单�?,
  `main_status`                 varchar(32)      NOT NULL                COMMENT '主状�?,
  `evaluate_status`             varchar(32)      NOT NULL                COMMENT '评价状�?,
  ...
  `return_method`               varchar(16)      DEFAULT NULL            COMMENT '机器返回方式（回�?自提�?,
  `return_express_no`           varchar(64)      DEFAULT NULL            COMMENT '回寄快递单�?,
...
CREATE TABLE IF NOT EXISTS `work_order_flow` (
  ...
  `before_status`       varchar(32)      DEFAULT NULL            COMMENT '动作前主状�?,
  `after_status`        varchar(32)      DEFAULT NULL            COMMENT '动作后主状�?,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='工单流转历史�?;
...
CREATE TABLE IF NOT EXISTS `work_order_participant` (
  ...
  `is_readonly`            tinyint unsigned DEFAULT 1               COMMENT '是否只读�?=是，0=否）',
  `first_participate_time` datetime         NOT NULL                COMMENT '首次参与时间',
  `last_participate_time`  datetime         NOT NULL                COMMENT '最后参与时�?,
  `create_time`            datetime         NOT NULL                COMMENT '创建时间',
  `update_time`            datetime         NOT NULL                COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
-- -------------------------------------------
-- 4. 新增表：work_order_user_participant
-- -------------------------------------------
...
CREATE TABLE IF NOT EXISTS `work_order_quote` (
  ...
  `is_current_valid` tinyint unsigned DEFAULT 1               COMMENT '是否当前有效报价�?=是，0=否）',
  `create_time`      datetime         NOT NULL                COMMENT '创建时间',
  `update_time`      datetime         NOT NULL                COMMENT '更新时间',
  PRIMARY KEY (`id`),
  KEY `idx_work_order_quote_time` (`work_order_id`, `create_time`),
  KEY `idx_quote_company` (`company_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='工单报价记录�?;

-- -------------------------------------------
-- 6. 新增表：work_order_repair
-- -------------------------------------------
CREATE TABLE IF NOT EXISTS `work_order_repair` (
  ...
  `is_finished`    tinyint unsigned DEFAULT 0               COMMENT '是否维修完成�?=是，0=否）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='工单维修登记�?;

-- -------------------------------------------
-- 7. 新增表：work_order_fault
-- -------------------------------------------
CREATE TABLE IF NOT EXISTS `work_order_fault` (
  ...
  `sort_num`      int unsigned     DEFAULT 0               COMMENT '排序�?,
...
CREATE TABLE IF NOT EXISTS `work_order_evaluation` (
  ...
  `satisfaction_score` tinyint unsigned DEFAULT NULL       COMMENT '服务满意度评�?,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='工单评价�?;
