...
/** 用户列表「角色」列单个 Tag 的展示数据 */
type UserRoleTagItem = {
  /** 列表 key，优先 roleKey + id，避免同名角色重复 */
  key: string;
  label: string;
  color: string;
};
...
/**
 * 作用：从用户行数据中解析当前公司下角色 Tag 列表（名称 + 区分色）。
 * @param record - 用户列表行
 * @returns 角色 Tag 项（无角色时为空数组）
 * @修改人 黄碧莲
 * @修改时间 2026-05-24
 */
function formatUserRoleTagItems(record: RowData): UserRoleTagItem[] {
