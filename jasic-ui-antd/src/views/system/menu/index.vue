...
const MENU_LIST_ACTION_COL_WIDTH = estimateAntTableActionColWidth([
  "编辑",
  "新增子级",
  "发布",
  "删除",
...
const menuFormDrawerWidth = computed(() =>
  adaptiveModalWidth(960, menuEditFieldCount.value)
);
...
const copyFormRules = {
  sourceSubjectType: [
    { required: true, message: "请选择源主体类型", trigger: "change" },
  ],
  targetSubjectType: [
...
const columns = computed(() => [
  ...
  { title: "操作", key: "actions", width: MENU_LIST_ACTION_COL_WIDTH, fixed: "right" as const },
]);
...
const groupedPublishTemplates = computed(() => {
  ...
  const typeLabelMap = publishOptions.value.typeOptions.reduce<
  ...
  const groupMap = publishOptions.value.templateOptions
    ...
    .reduce<
      Record<
        string,
        { typeCode: string; typeName: string; templates: RowData[] }
      >
    >((map, item) => {
...
const publishFormRules = computed(() => ({
  ...
  targetTemplateIds: [
    {
      validator: async () => {
        const filteredTemplates = publishOptions.value.templateOptions.filter(
          (item) => publishForm.targetTypeCodes.includes(String(item.typeCode)),
        );
        if (
          filteredTemplates.length > 0 &&
          !publishForm.targetTemplateIds.length
        ) {
...
async function loadMenuOptions() {
  const { data } = await menuTree(
    String(formModel.subjectType || subjectType.value),
  );
  menuOptions.value = [
    { id: 0, menuName: "顶级菜单", children: normalizeMenuTree(data) },
  ];
}
  ...
  try {
    ...
    if (
      !notifyOnceSuccessFromFlatResult(
        res,
        `拷贝成功，共 ${copyCheckedKeys.value.length || 0} 个菜单`,
      )
    ) {
      return;
    }
...
async function openPublishDialog(menu: RowData, returnToForm: boolean) {
  ...
  try {
    ...
    const templateOptions = Array.isArray(obj.templateOptions)
      ? obj.templateOptions
      : [];
    publishOptions.value = { typeOptions, templateOptions };
    publishForm.targetTypeCodes = typeOptions.map((item) =>
      String(item.typeCode),
    );
...
function toggleExpand() {
  ...
  expandedRowKeys.value = isExpandAll.value
    ? collectRowKeys(menuTreeRows.value)
    : [];
}
      ...
      <template #extra>
        <ASpace>
          ...
          <AButton
            ...
            >新增</AButton
          >
          <AButton v-if="hasAuth('system:menu:add')" @click="openCopyDialog"
            >菜单拷贝</AButton
          >
        </ASpace>
      </template>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'menuType'">
            ...
            <ATag v-else-if="record.menuType === 'C'" color="success"
              >菜单</ATag
            >
            <ATag v-else-if="record.menuType === 'F'" color="processing"
              >按钮</ATag
            >
            <span v-else>-</span>
          </template>
        ...
        <ARow :gutter="16">
          ...
          <ACol :span="12">
            <AFormItem label="主体类型" name="subjectType" required>
              <ASelect
                v-model:value="formModel.subjectType"
                :options="subjectTypeOptions"
                @change="loadMenuOptions"
              />
            </AFormItem>
          </ACol>
          <ACol :span="12">
    ...
    <ADrawer v-model:open="copyOpen" title="菜单拷贝" :width="360">
      ...
      >
        ...
        <AFormItem label="目标主体类型" name="targetSubjectType" required>
          <ASelect
            v-model:value="copyForm.targetSubjectType"
            :options="subjectTypeOptions"
          />
        </AFormItem>
      ...
      <template #footer>
        <ASpace :size="16">
          ...
          <AButton type="primary" :loading="copySubmitting" @click="submitCopy"
            >开始拷贝</AButton
          >
        </ASpace>
      </template>
        ...
        >
          ...
          <AFormItem label="主体类型">
            <span>
              {{
                subjectTypeLabelMap[
                  publishForm.menu?.subjectType as "PLATFORM" | "HQ" | "SERVICE"
                ] || "-"
              }}
            </span>
          </AFormItem>
          <AFormItem label="目标公司类型" name="targetTypeCodes" required>
            <ACheckboxGroup
              v-model:value="publishForm.targetTypeCodes"
              @change="onPublishTypeCodesChange"
            >
          ...
          <AFormItem
            label="目标角色模板"
            name="targetTemplateIds"
            :required="groupedPublishTemplates.length > 0"
          >
            <div
              v-if="groupedPublishTemplates.length === 0"
              class="publish-empty"
            >
            ...
            <div v-else class="publish-template-list">
              <div
                v-for="group in groupedPublishTemplates"
                :key="group.typeCode"
                class="publish-template-group"
              >
                <div class="publish-template-title">
                  {{ group.typeName }}（{{ group.typeCode }}）
                </div>
          ...
          <AFormItem label="同步已有公司">
            ...
            <span class="publish-tip"
              >仅向匹配模板 `roleKey` 的系统角色追加当前菜单。</span
            >
          </AFormItem>
        </AForm>
      </ASpin>
